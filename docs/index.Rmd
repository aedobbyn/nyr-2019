...
`status_id` is useful because we can supply a `max_id` to get only tweets older than that ID 

--

--> good for working backwards in time to get older and older data `r emo::ji("thumbsup")`
...
To test if `get_tweets` can start with a `tbl` of tweets and look for new ones,
